; ════════════════════════════════════════════════════════════════════════════════
; Baxley Status API — Windows Installer
...
; ── Metadata ──────────────────────────────────────────────────────────────────
!define APP_NAME      "Baxley Status API"
!define APP_VERSION   "1.0.0"
!define APP_PUBLISHER "Trust Biz Tech"
!define SVC_NAME      "BaxleyStatusAPI"
!define UNREG_KEY     "Software\Microsoft\Windows\CurrentVersion\Uninstall\${SVC_NAME}"

Name             "${APP_NAME} ${APP_VERSION}"
OutFile          "../../dist/BaxleyStatusAPI-Setup.exe"
InstallDir       "$PROGRAMFILES64\BaxleyStatusAPI"
...
Function .onInit
  ...
  ${If} $DbPath == ""
    StrCpy $DbPath "C:\path\to\GreenTally.sqlite"
  ${EndIf}
  ${If} $MqttHost == ""
...
; ── Config page ───────────────────────────────────────────────────────────────
Function ConfigPage
  !insertmacro MUI_HEADER_TEXT "Pipeline Configuration" "Set the database location and MQTT broker connection."
  ...
  ${NSD_CreateLabel} 0 0u 100% 10u "Database path (absolute path to the SQLite file written by ETLGreenTally.ps1):"
  Pop $0
  ${NSD_CreateText} 0 12u 100% 13u $DbPath
  Pop $DbPathCtrl

  ${NSD_CreateLabel} 0 32u 100% 10u "ETL base path (shared folder containing CSV, Excel, and log subfolders — leave blank if unused):"
  Pop $0
...
Function ConfigPageLeave
  ...
  ${If} $DbPath == ""
    MessageBox MB_OK|MB_ICONEXCLAMATION "Database path is required."
    Abort
  ${EndIf}
FunctionEnd
...
Section "${APP_NAME}" SEC_MAIN
  ...
  FileWrite $0 "CSV_NEW_PATH=New$\r$\n"
  FileWrite $0 "CSV_FINISHED_PATH=finished$\r$\n"
  FileWrite $0 "EXCEL_BASE_PATH=base$\r$\n"
  FileWrite $0 "EXCEL_TOTAL_PATH=total$\r$\n"
  FileWrite $0 "EXCEL_FINAL_PATH=Final$\r$\n"
  FileWrite $0 "LOG_PATH=logs$\r$\n"
  ...
  FileWrite $0 "  <description>ETL health monitor - Baxley Green Tally pipeline</description>$\r$\n"
