...
DIST="$ROOT/dist"
INSTALLER_DIR="$ROOT/baxley-status-api/installer"
...
cp -r "$ROOT/baxley-status-api/src"           "$STAGING/app/"
mkdir -p "$STAGING/app/config" "$STAGING/app/data"
cp "$ROOT/baxley-status-api/package.json"      "$STAGING/app/"
cp "$ROOT/baxley-status-api/package-lock.json" "$STAGING/app/"
...
cp "$WINSW_TMP" "$STAGING/BaxleyStatusAPI.exe"
...
cd "$INSTALLER_DIR"
makensis BaxleyStatusAPI.nsi
cd "$ROOT"

echo ""
echo "Done → $DIST/BaxleyStatusAPI-Setup.exe"
